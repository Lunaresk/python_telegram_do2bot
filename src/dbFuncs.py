...
def initDB():
    with getCur() as cur:
        cur.execute(
            "CREATE TABLE IF NOT EXISTS Users(TId BIGINT PRIMARY KEY NOT NULL, Lang CHAR(2) NOT NULL DEFAULT 'en');")
        cur.execute(
        ...
        cur.execute(
            "CREATE TABLE IF NOT EXISTS InSettings(List TEXT NOT NULL REFERENCES Lists(Code) ON UPDATE CASCADE ON DELETE CASCADE, Member BIGINT NOT NULL REFERENCES Users(TId) ON UPDATE CASCADE ON DELETE CASCADE);")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS Notify(List TEXT NOT NULL REFERENCES Lists(Code) ON UPDATE CASCADE ON DELETE CASCADE, Member BIGINT NOT NULL REFERENCES Users(TId) ON UPDATE CASCADE ON DELETE CASCADE);")
        cur.execute(
        ...
        cur.execute(
            "CREATE TABLE IF NOT EXISTS Coworkers(List TEXT NOT NULL REFERENCES Lists(Code) ON UPDATE CASCADE ON DELETE CASCADE, Worker BIGINT NOT NULL REFERENCES Users(Tid) ON UPDATE CASCADE ON DELETE CASCADE, Name TEXT NOT NULL, Message TEXT NOT NULL DEFAULT '0');")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS InlineMessages(List TEXT NOT NULL REFERENCES Lists(Code) ON UPDATE CASCADE ON DELETE CASCADE, InlineID TEXT NOT NULL);")
        cur.execute(
            "CREATE TABLE IF NOT EXISTS PrivateInlines(InlineID TEXT PRIMARY KEY NOT NULL, MessageID BIGINT NOT NULL);")
